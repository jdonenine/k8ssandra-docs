...
### Grafana
A multi-platform open source analytics and interactive visualization web application. It provides charts, graphs, and alerts for the web when connected to supported data sources. K8ssandra provides preconfigured Grafana dashboards that visualize Cassandra, cluster, OS and node metrics that are captured at runtime by [Prometheus]({{< relref "#prometheus" >}}) (also provided by K8ssandra). See the [Metrics Collector]({{< relref "/v1/components/metrics-collector" >}}) component, and the monitor [task]({{< relref "/v1/tasks/monitor/" >}}).
...
### K8ssandra
An open source, production-ready platform for running Apache Cassandra® on Kubernetes. [K8ssandra](https://k8ssandra.io) includes automation for operational tasks such as [repairs]({{< relref "/v1/tasks/repair" >}}), [backup/restore]({{< relref "/v1/tasks/backup-restore" >}}), and [monitoring]({{< relref "/v1/tasks/monitor" >}}).

### Medusa
An open source backup and restore tool for Cassandra data, deployed by K8ssandra for Kubernetes environments. For more, see [Medusa component]({{< relref "/v1/components/medusa/" >}}) and [backup and restore tasks]({{< relref "/v1/tasks/backup-restore/" >}}). 
...
### MinIO
An Amazon S3-compatible server-side software storage stack. MinIO is one of the local or cloud-based storage objects ("buckets") supported by K8ssandra's Medusa backup/restore operations. For more, see [Backup and restore with MinIO]({{< relref "/v1/tasks/backup-restore/minio/" >}}) buckets.
...
### nodetool
A Cassandra [command-line interface](https://cassandra.apache.org/doc/latest/tools/nodetool/nodetool.html) for monitoring a cluster and performing routine database operations. It is typically run from an operational node, and includes commands such as `nodetool repair`. For repair operations in Kubernetes, K8ssandra recommends an alternative: [Reaper]({{< relref "/v1/components/reaper" >}}), which is deployed by K8ssandra. Also see the Cassandra [repair tasks]({{< relref "/v1/tasks/repair" >}}).
...
* [Set up port forwarding]({{< relref "/v1/quickstarts/developer/#set-up-port-forwarding" >}}).  
...
For details, see the K8ssandra documentation topics covering the [Reaper component]({{< relref "/v1/components/reaper" >}}) and [repair tasks]({{< relref "/v1/tasks/repair" >}}).

### repair
In the context of Cassandra data, anti-entropy is the process of comparing the data of all replicas, and updating each replica to the newest version. Cassandra has two phases to the process: Build a Merkle tree for each replica, and then compare the Merkle trees to discover differences. K8ssandra deploys Reaper to your Kubernetes environment. See [Repair Cassandra with Reaper]({{< relref "/v1/tasks/repair" >}}).
...
### secret
A way to store and manage sensitive information, such as passwords, OAuth tokens, and ssh keys. Storing confidential information in a secret is safer and more flexible than putting it verbatim in a Pod definition or in a container image. For more, see [K8ssandra security]({{< relref "/v1/tasks/secure" >}}).

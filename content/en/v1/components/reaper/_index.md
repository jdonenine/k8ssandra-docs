...
Reaper for Apache Cassandra&reg; is deployed by a K8ssandra install, which includes the Reaper Operator [Helm chart]({{< relref "/v1/reference/helm-charts/reaper-operator/" >}}).
...
Here's an example from the Reaper Web UI. For more, see [Repair tasks]({{< relref "/v1/tasks/repair/" >}}). 
...
* For the steps to set up repair operations using the Reaper Web UI, see [Repair Cassandra with Reaper]({{< relref "/v1/tasks/repair/" >}}). 
* For information about using a superuser and secrets with Reaper authentication, see [Reaper security]({{< relref "/v1/tasks/secure/#reaper-security" >}}).
* For reference details, see the Reaper Operator [Helm chart]({{< relref "/v1/reference/helm-charts/reaper-operator/" >}}).

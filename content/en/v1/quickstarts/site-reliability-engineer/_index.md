...
Be sure to first complete one of the [K8ssandra install]({{< relref "/v1/install" >}}) options (locally or on a cloud provider) before performing these post-install steps. 
{{% /alert %}}
...
In order to access Cassandra utilities outside of the K8s cluster, if you don't have an Ingress setup as described in [Configure Ingress]({{< relref "/v1/tasks/connect/ingress" >}}), you'll need to configure port forwarding.
...
[Reaper](http://cassandra-reaper.io/) is an easy interface for managing K8ssandra cluster repairs.  Reaper is deployed as part of the K8ssandra [install]({{< relref "/v1/install" >}}). 

![Reaper](cass-reaper.png)

For details, start in the [Reaper]({{< relref "/v1/components/reaper" >}}) topic. Then read about the [repair]({{< relref "/v1/tasks/repair" >}}) tasks you can perform with Reaper.
